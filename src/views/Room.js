...
import { View, ImageBackground, StyleSheet, Text, Image, TouchableOpacity, Alert } from 'react-native';
...
const Room = (props) => {
    const [tmp, setTmp] = useState(4)
    const [indexMode, setIndexMode] = useState(props.route.params.indexMode)
    ...
    const content = (tmp) => {
        if (tmp === 4) {
            return <Text style={styles.textCompteur}>Trouver le nom du titre en appuyant sur le buzzer</Text>
        } else if (tmp != 0) {
    ...
    return (
        <View style={styles.container}>
            {numberTitle !== 5 ? tmp === 0 ?
                <ImageBackground source={gifs[indexMode]} style={styles.content} resizeMode='stretch'>
                    ...
                    <View style={styles.viewTitle}>
                        ...
                        <Button icon="music" iconFamily="Feather" color="red" size="small" capitalize shadowless >TITRE 1/2</Button>
                    </View>

                    {/* setNumberTitle(numberTitle + 1) */}
                    {numberTitle !== 5 && (
                        <TouchableOpacity style={styles.viewBuzzer} onPress={() => setOpenModal(true)}>
                            <Image
            ...
            <ResponseTitle visible={openMoal} nextSong={nextSong}>
            </ResponseTitle>
